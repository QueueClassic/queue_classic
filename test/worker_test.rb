...
class WorkerTest < QCTest
  ...
  def test_worker_can_work_multiple_queues
    ...
    worker = TestWorker.new(q_names: %w[priority_queue secondary_queue])
  ...
  def test_worker_works_multiple_queue_left_to_right
    ...
    worker = TestWorker.new(q_names: %w[left_queue right_queue])
  ...
  def test_worker_unlocks_job_on_signal_exception
    ...
    fake_unlock = proc do |job_id|
      unlocked = true if job_id == job_details['id']
