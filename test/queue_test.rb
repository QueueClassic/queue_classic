...
class QueueTest < QCTest
  ...
  def test_lock_with_future_job_with_enqueue_at_with_a_time_object
    ...
    assert_nil QC.lock
    sleep 0.1 until Time.now >= future
    job = QC.lock
    assert_equal('Klass.method', job[:method])
    assert_equal([], job[:args])
  ...
  def test_repair_after_error
    ...
    queue.conn_adapter.connection.stub :exec, ->(*_args) { raise(PG::Error) } do
      queue.conn_adapter.connection.stub :reset, ->(*_args) { raise(ResetError) } do
  ...
  def test_enqueue_in_stops_retrying_on_permanent_error
    ...
    begin
      queue.conn_adapter.connection.exec('select pg_terminate_backend(pg_backend_pid())')
    rescue StandardError
      nil
    end
